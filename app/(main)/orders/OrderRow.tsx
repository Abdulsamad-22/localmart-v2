...
export function OrderRow({ item }: Props) {
  ...
  return (
    <Link
      href={`/orders/${item.id}`}
      className={`flex items-center gap-4 p-4 rounded-xl border transition-all duration-150 hover:border-gray-300 dark:hover:border-gray-700 hover:shadow-sm
        ${
          !item.is_read
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-gray-400 mt-0.5">
          Ref: {item.orders[0].payment_reference}
        </p>
      </div>
